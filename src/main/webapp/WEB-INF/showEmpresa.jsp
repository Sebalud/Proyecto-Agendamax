...
<body class="container">
    ...
    <!-- Contenido-->
    <h1>Bienvenido
        <c:out value="${empresa.usuarioAdmin.nombre}" />
    </h1>
            ...
            <form id="form" class="alinearDisplay ancho mt-5 ms-5" method="POST" action="/plan/${empresa.id}">
                <select class="form-select" id="servicioElegido" name="servicio">
                    ...
                    <c:forEach items="${serviciosNotEmpresa}" var="servicio">
                        <option value="${servicio.id}">${servicio.servicioOfrecido} duracion:
                            ${servicio.duracionServicio} min</option>
                    </c:forEach>
                    <option value="opcionEspecial"><button id="miBoton">Haz clic aquí para mostrar el input</button>
                    </option>
                </select>
